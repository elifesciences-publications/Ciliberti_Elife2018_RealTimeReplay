...
CovarianceMatrix* covariance_create( uint16_t ndim ) {
    ...
    CovarianceMatrix* m = malloc( sizeof(CovarianceMatrix) );
    m->refcount = 1;
    
    m->ndim = ndim;
    
    m->data = malloc( ndim*ndim*sizeof(double) );
...
CovarianceMatrix* covariance_load( FILE* in ) {
    ...
    if ( fread( m->data, sizeof(double), ndim*ndim, in ) != ndim*ndim ) {} //TODO
...
void covariance_alloc_sigma( CovarianceMatrix* m ) {
    ...
    if (m->sigma.points == NULL)
    {
        ...
        m->sigma.points = malloc( m->sigma.n * m->ndim * sizeof(double) );
    }
}
