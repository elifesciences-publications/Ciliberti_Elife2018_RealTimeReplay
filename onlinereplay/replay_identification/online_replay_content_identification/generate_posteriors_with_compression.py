...
# read data
_, ephys, start_stop = read_dataset( config.path_to_dataset ) 
...
epoch = seg.fromarray( seg.Segment( [start_stop[0], start_stop[1]] ) )
...
epoch_split = epoch.split( size=config.bin_size_replay )

posteriors, _, n_spikes_event, n_tt_ext_event =\
    decoder.decode( spike_features, epoch_split, tt_included,\
...
if config.save_posteriors:
    np.save( open( join( dirname( config.path_to_dataset ),\
        "posteriors_compressed_decoding_epoch.npy" ), 'w'), posteriors )
    print "\nPosteriors saved"
