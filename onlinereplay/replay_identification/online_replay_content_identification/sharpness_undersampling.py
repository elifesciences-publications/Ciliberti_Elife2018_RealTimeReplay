...
import sharpness_undersampling_config as config
...
sharpness_values_with = np.zeros( (n_bins_with, n_bootstrap) )
sharpness_avg_values_with = np.zeros( (n_bins_with, n_bootstrap) )
sharpness_values_without = np.zeros( (n_bins_without, n_bootstrap) )
sharpness_avg_values_without = np.zeros( (n_bins_without, n_bootstrap) )
sharpness_values_outside = np.zeros( (n_bins_outside, n_bootstrap) )
sharpness_avg_values_outside = np.zeros( (n_bins_outside, n_bootstrap) )
...
for i,combo in enumerate(itertools.combinations( range(n_tt), config.n_removed )):
    ...
    sharpness_values_with[:, j], sharpness_avg_values_with[:,j] =\
        decoding_machine.compute_sharpness_combo( combo, candidate_ext_split_with,\
        lazy_decode=True )
    sharpness_values_without[:, j], sharpness_avg_values_without[:,j] =\
        decoding_machine.compute_sharpness_combo( combo, candidate_ext_split_without,\
        lazy_decode=True )
    sharpness_values_outside[:, j], sharpness_avg_values_outside[:,j] =\
        decoding_machine.compute_sharpness_combo( combo, non_candidate_ext_split,\
        lazy_decode=True )
...
if config.save_values:
    ...
    filename = join( config.path_to_results, "sharpness_with_" + suffix )
    np.save( open(filename, 'w'), sharpness_values_with )
    filename = join( config.path_to_results, "sharpness_without_" + suffix )
    np.save( open(filename, 'w'), sharpness_values_without )
    filename = join( config.path_to_results, "sharpness_outside_" + suffix )
    np.save( open(filename, 'w'), sharpness_values_outside )
    filename = join( config.path_to_results, "sharpness_avg_with_" + suffix )
    np.save( open(filename, 'w'), sharpness_avg_values_with )
    filename = join( config.path_to_results, "sharpness_avg_without_" + suffix )
    np.save( open(filename, 'w'), sharpness_avg_values_without )
    filename = join( config.path_to_results, "sharpness_avg_outside_" + suffix )
    np.save( open(filename, 'w'), sharpness_avg_values_outside )
